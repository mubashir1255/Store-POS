  ...
  return (
    <div className="page-grid">
      <div className="panel" style={{ padding: '1rem' }}>
        ...
        <button type="button" className="btn btn-primary" onClick={save} style={{ marginTop: '0.75rem' }} disabled={form.username === 'admin' && !form.id}>
          ...
          <tbody>
            {list.map((u) => (
              <tr key={u.id}>
                ...
                <td>
                  {u.username === 'admin' ? (
                    <span className="status-pill" style={{ background: 'var(--warn)', color: '#000' }}>System Protected</span>
                  ) : (
                    <button
                      type="button"
                      className="btn"
